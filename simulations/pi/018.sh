...
echo
echo "Please log into rasp-36 and run ./simulations/pi/036.sh up to this point"
echo "The minimal set of public keys must then be exchanged with rasp-036:"
echo
echo "  scp \"$tmp/F.pub.pem\" \"rasp-036:$tmp\""
echo
read -p "Press Enter to continue once this is completed..." _
...
echo "  TCDICN_PORT=33336 TCDICN_DPORT=33333 TCDICN_ID=C TCDICN_KEYFILE=\"$tmp/C.pem\" TCDICN_GROUPS=\"1:$tmp/E.pub.pem\" PYTHONPATH=. python3 applications/drone.py"
...
echo "  TCDICN_PORT=33338 TCDICN_DPORT=33333 TCDICN_ID=E TCDICN_KEYFILE=\"$tmp/E.pem\" TCDICN_GROUPS=\"1:$tmp/A.pub.pem,$tmp/C.pub.pem,$tmp/F.pub.pem\" KNOWN_DRONES=\"A,C,H\" PYTHONPATH=. python3 applications/inspector.py"
echo "  TCDICN_PORT=33339 TCDICN_DPORT=33333 TCDICN_ID=F TCDICN_KEYFILE=\"$tmp/F.pem\" TCDICN_GROUPS=\"1:$tmp/E.pub.pem,$tmp/L.pub.pem 2:$tmp/D.pub.pem,$tmp/L.pub.pem\" KNOWN_DRONES=\"A,B,C,G,H,I\" PYTHONPATH=. python3 applications/controller.py"
